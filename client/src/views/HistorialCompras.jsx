...
export const HistorialCompras = () => {
    const { pedidosDelCliente } = useOrders();
    const { user } = useUser();

    const misPedidos = pedidosDelCliente(user?.id_cliente);
                    ...
                    <div className="profile-content">
                        ...
                        {misPedidos.length === 0 ? (
                            <div className="text-center py-5">
                                <i className="fa-regular fa-bag-shopping fa-3x mb-3" style={{ color: '#6B705C' }}></i>
                                <h5 className="fw-semibold">Aún no tienes pedidos</h5>
                                <p className="text-secondary">Cuando realices una compra, aparecerá aquí.</p>
                                <Link to="/libros" className="btn boton-primario mt-2">Ver libros</Link>
                            </div>
                        ) : (
                                ...
                                <div key={pedido.id_pedido}>
                                    ...
                                    <div className="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2 py-3">
                                        <div>
                                            ...
                                            <p className="text-secondary mb-1">Fecha: {pedido.fecha}</p>
